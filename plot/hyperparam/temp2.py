...
def plotPerformances(realvalues, modelvalues_lst, plt, rankingUnderModel_lst, rankingUnderReal, model_key_lst, title, compareVal=None, compareRank=None):
    ...
    for j in range(len(modelvalues_lst)):
        ...
        plt.scatter([i+1 for i in range(len(modelvalues_lst[j]))], np.array(modelvaluesRankedByRealRanking)/scale,
                    label='Performance: model drop={}'.format(key), s=5, color=cmap(j/len(modelvalues_lst)))
...
def all_performance(model_parents_lst, real_parent, vanilla=None, title="default"):
    ...
    for model_parents in model_parents_lst:
        ...
        label_key_lst.append(model_parents[0].split("/drop")[1].split("/")[0])
...
vanilla_parent = None
all_performance([k1_drop2], real_parent, title="drop0.2")
print("\n\n\n")
print("2nd func")
all_performance([k1_drop1, k1_drop2], real_parent, title="knn1")