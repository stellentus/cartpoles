...
def plotPerformances(realvalues, modelvalues_lst, plt, rankingUnderModel_lst, rankingUnderReal, model_key_lst, title, compareVal=None, compareRank=None):
    ...
    for j in range(len(modelvalues_lst)):
        ...
        plt.scatter([i+1 for i in range(len(modelvalues_lst[j]))], np.array(modelvaluesRankedByRealRanking)/scale,
                    label='Performance: model k={}'.format(key), s=5, color=cmap(j/len(modelvalues_lst)))

        max_idx = np.array(modelvaluesRankedByRealRanking).argmax()
        print(modelvaluesRankedByRealRanking, max_idx, modelvaluesRankedByRealRanking[max_idx])
        plt.scatter([max_idx+1], np.array(modelvaluesRankedByRealRanking[max_idx]/scale), color="green", s=5)#, label='Hyperparameter\nchosen by model')
        plt.scatter([max_idx+1], np.array(modelvaluesRankedByRealRanking[max_idx]/scale), facecolors='none', edgecolors="green", s=160)
...
def all_performance(model_parents_lst, real_parent, vanilla=None, title="default"):
    ...
    for model_parents in model_parents_lst:
        for _, performance in enumerate(model_parents):
        ...
        print(modelkeys)
        print(modelvalues)
        print(rankingUnderModel)

        modelvalues_lst.append(modelvalues)
        rankingUnderModel_lst.append(rankingUnderModel)
        label_key_lst.append(model_parents[0].split("knn-ensemble/k")[1].split("/")[0])
...
k1_drop0 = [
    "../../data/hyperparam/cartpole/offline_learning/knn-ensemble/k1/esarsa/step7.5k_env/lockat_baseline/drop0.1/ensembleseed0/",
]
k3_drop0 = [
    "../../data/hyperparam/cartpole/offline_learning/knn-ensemble/k3/esarsa/step7.5k_env/lockat_baseline/drop0.1/ensembleseed0/",
]
k5_drop0 = [
    "../../data/hyperparam/cartpole/offline_learning/knn-ensemble/k5/esarsa/step7.5k_env/lockat_baseline/drop0.1/ensembleseed0/",
]

vanilla_parent = None
# all_performance([k1_drop0, k3_drop0, k5_drop0], real_parent, vanilla_parent, title="drop0")
all_performance([k1_drop2, k1_drop1], real_parent, vanilla_parent, title="drop0.2")
print("\n\n\n")
print("2nd func")
all_performance([k1_drop1, k1_drop2], real_parent, vanilla_parent, title="knn1")

# all_performance([k1_drop1, k3_drop1, k5_drop1], real_parent, vanilla_parent, title="drop0.1")
#
# all_performance([k1_drop2, k3_drop2, k5_drop2], real_parent, vanilla_parent, title="drop0.2")
#
# all_performance([k1_drop3, k3_drop3, k5_drop3], real_parent, vanilla_parent, title="drop0.3")