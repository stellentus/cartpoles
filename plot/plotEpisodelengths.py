...
labels = ['esarsa_acrobot']
dataPath = ['esarsa_acrobot/param_0/']
...
for i in range(len(dataPath)):
    ...
    Data[algorithms[i]] = Data[algorithms[i]][:,1::2]
    ...
    for alg, data in Data.items():
        ...
        for j in range(0, len(xAxis), plotwindow):
            ...
            if j + plotwindow >= len(xAxis):
                mean = getMean(tempdata)
                plt.plot(tempxAxis, mean, label=labels[i], color=colors[i])
                lowerBound, upperBound = getConfidenceIntervalOfMean(data, confidence=0.95)
                plt.fill_between(xAxis, lowerBound, upperBound, alpha=0.25, color=colors[i])
                #lowerRun, upperRun = getRegion(data, lower=0.0, upper=1.0, transformation='Returns', type='pertimestep')
                #plt.fill_between(xAxis, lowerRun, upperRun, alpha=0.25, color=colors[i])
            else:
                mean = getMean(tempdata)
                plt.plot(tempxAxis, mean, label='', color=colors[i])
                lowerBound, upperBound = getConfidenceIntervalOfMean(data, confidence=0.95)
                plt.fill_between(xAxis, lowerBound, upperBound, alpha=0.25, color=colors[i])
                #lowerRun, upperRun = getRegion(data, lower=0.0, upper=1.0, transformation='Returns', type='pertimestep')
                #plt.fill_between(xAxis, lowerRun, upperRun, alpha=0.25, color=colors[i])
...
plt.tight_layout()
#plt.show()
plt.savefig('../img/esarsa-sgd-acrobotCI.png',dpi=500, bbox_inches='tight')
