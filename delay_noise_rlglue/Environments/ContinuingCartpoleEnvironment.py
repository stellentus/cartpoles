...
class CartpoleEnvironmentContinuing(BaseEnvironment):
	def __init__(self, seed):
		...
		self.actual_index = [0 for i in range(4)]
		self.delay_index = [0 for i in range(4)]
		self.delays = [10,10,10,10]
		self.delay_observations = None
		np.random.seed(self.seed)
		self.percent_noise = [10.0, 10.0, 10.0, 10.0]
	...
	def step(self, action):
		...
		lows = [-2.4, -4.0, -(12 * 2 * math.pi / 360), -3.5]
		highs = [2.4, 4.0, (12 * 2 * math.pi / 360), 3.5]
		
		noise = [np.random.uniform(low=self.percent_noise[i]*lows[i], high=self.percent_noise[i]*highs[i]) for i in range(len(lows))]
		#noise = [0,0,0,0]
