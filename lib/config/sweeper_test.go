...
func newTestRlglueAttributesSweep() rlglue.Attributes {
...
func newTestRlglueAttributesList() rlglue.Attributes {
	return rlglue.Attributes(`{
		"name": "test",
		"gamma": 0.9,
		"alpha": 15,
		"list": [
			...
			{"fqi-hidden": [4, 4]},
			{"fqi-hidden": [4, 4], "epsilon": 0, "alpha": 1e-3},
			{"fqi-hidden": [16, 16], "epsilon": 0}
		]
	}`)
}

func TestAttrAsJson(t *testing.T) {
	raw := newTestRlglueAttributesSweep()
	out := AttributeMap{}
...
func TestSweeperLoadSweep(t *testing.T) {
	sw := sweeper{}
	err := sw.Load(newTestRlglueAttributesSweep())
	assert.NoError(t, err)
...
func TestSweeperLoadList(t *testing.T) {
	sw := sweeper{}
	err := sw.Load(newTestRlglueAttributesList())
	assert.NoError(t, err)

	expected := []AttributeMap{
		AttributeMap{"name": &name, "gamma": &gamma, "alpha": &alpha3, "fqi-hidden": &fqi4, "epsilon": &epsilon},
		AttributeMap{"name": &name, "gamma": &gamma, "alpha": &alpha3, "fqi-hidden": &fqi4},
		AttributeMap{"name": &name, "gamma": &gamma, "alpha": &alpha4, "fqi-hidden": &fqi4, "epsilon": &epsilon},
		AttributeMap{"name": &name, "gamma": &gamma, "alpha": &alpha, "fqi-hidden": &fqi4},
		AttributeMap{"name": &name, "gamma": &gamma, "alpha": &alpha3, "fqi-hidden": &fqi4, "epsilon": &epsilon},
		AttributeMap{"name": &name, "gamma": &gamma, "alpha": &alpha, "fqi-hidden": &fqi16, "epsilon": &epsilon},
	}
	assert.Equal(t, expected, sw.allAttributes)
}
