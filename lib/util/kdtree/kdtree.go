...
func (t *TransTrees)BuildTree(allTrans [][]float64) {
	var action int
	for i:=0; i<len(allTrans[0]); i++ {
		action = int(allTrans[i][t.stateDim])
		t.data[action] = append(t.data[action], allTrans[i][: t.stateDim]) // sort current state
	}
	for i:=0; i<t.numAction; i++ {
		err := t.trees[i].Build(t.data[i])
		if err != nil {
			panic("Error when building tree for action "+string(i))
		}
	}
}
...
func (t *TransTrees) SearchTree(target []float64, action int, k int) ([][]float64, [][]float64, []float64, []float64, []float64){
	...
	for i:=0; i<k; i++ {
		...
		gammas[i] = t.data[action][neighborIdxs[i]][t.stateDim*2+2]
	}
	return states, nextStates, rewards, gammas, dists
}
